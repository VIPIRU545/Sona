function saveinstanceseralizer(...)
    ...
    <Properties>\
    <string name="Name">Sona_Save</string>\
    <ProtectedString name="Source">'..[[]]..'</ProtectedString>\
    			...
    			print("SaveInstance: Map has been Saved check ur workspace folder", Color3.new(0, 0, 44/255))
    			resetState()
    		end
    	}
    ...
    return serializer.SaveInstance(...)
end
getgenv().saveinstance = function(saveinsname, saveinstanceset)
	if not saveinsname then saveinsname = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name.."" end
	saveinstanceseralizer(game,saveinsname, saveinstanceset)
end
